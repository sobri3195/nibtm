...
export default function Datasets({ data }) {
  ...
  const items = useMemo(() => data.datasets.filter((item) => category === 'All' || item.categoryKey === category), [data.datasets, category])
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {items.map((item) => (
            <Card key={item.name} className="hover:-translate-y-1 hover:shadow-xl">
              ...
              <Button variant="secondary" className="mt-4 w-full">{data.ui.openDataset}</Button>
            </Card>
          ))}
        </div>
      </div>
